...
Appearance settings (theme, custom JSON theme, UI/mono font family, font sizes, syntax-highlight theme) are owned by `packages/app/src/appearance`. Its provider subscribes once and synchronizes Unistyles when settings or plugin contributions change. `applyAppearance` patches every key in `REGISTERED_THEMES` plus `custom`, returning `{ ...theme, fontFamily, fontSize, lineHeight, colors.syntax }`.
